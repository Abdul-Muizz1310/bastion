...
// Mock global fetch for demo steps
...
describe("10-demo: workflow", () => {
  ...
  it("startDemoRun returns a run_id (integration: needs running services)", async () => {
    const { startDemoRun } = await import("./demo");
  ...
  it("demo run executes 5 steps in order (integration)", async () => {
    const { startDemoRun } = await import("./demo");
    const result = await startDemoRun({ userId: "user-1", role: "admin" });
  ...
  it("each step event includes step name, status, and data (integration)", async () => {
    const { startDemoRun } = await import("./demo");
    const result = await startDemoRun({ userId: "user-1", role: "editor" });
  ...
  it("all demo events share the same requestId (integration)", async () => {
  ...
  it("demo results are retrievable by runId (integration)", async () => {
    const { startDemoRun } = await import("./demo");
    const result = await startDemoRun({ userId: "user-1", role: "admin" });
...
describe("10-demo: edge and failure cases", () => {
  ...
  it("concurrent demo runs get independent run_ids (integration)", async () => {
    mockFetch.mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ result: "ok" }),
    });
    ...
    const [r1, r2] = await Promise.all([
      startDemoRun({ userId: "user-1", role: "admin" }),
      startDemoRun({ userId: "user-2", role: "admin" }),
    ]);
  ...
  it("SSE reconnection gets remaining events (client-side test)", () => {
    ...
    // We verify the demo module produces the timeline data that would be streamed.
...
describe("10-demo: security", () => {
  ...
  it("all downstream calls use the gateway proxy (integration)", async () => {
    const { startDemoRun } = await import("./demo");
    await startDemoRun({ userId: "user-1", role: "admin" });
  ...
  it("demo run events appear in audit log (integration)", async () => {
    const { appendEvent } = await import("./audit");
    const { startDemoRun } = await import("./demo");
    await startDemoRun({ userId: "user-1", role: "admin" });
    ...
    // Each call includes the demo entityType
    for (const call of calls) {
      expect(call[0].entityType).toBe("demo");
    }
  });
});

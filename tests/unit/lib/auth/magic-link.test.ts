...
describe("02-auth: demo mode", () => {
  ...
  it("demo mode renders 3 buttons when DEMO_MODE=true (component test)", async () => {
    ...
    const { roleSchema } = await import("@/lib/validation");
...
describe("13-return-to: sendMagicLink returnTo integration", () => {
  let mocks: Record<string, Mock>;
  ...
  beforeEach(async () => {
    ...
    // Force email send to fail so sendMagicLink returns the raw URL (not "[redacted]")
    const resendMod = await import("resend");
    (resendMod as unknown as { Resend: unknown }).Resend = class {
      emails = {
        send: vi.fn().mockRejectedValue(new Error("email disabled for url inspection")),
      };
    };
  });

  it("case 28: safe returnTo is appended to URL (URL-encoded)", async () => {
    const { sendMagicLink } = await import("@/lib/auth/magic-link");
    const result = await sendMagicLink("user@example.com", "/dossiers/abc");
    expect(result.emailSent).toBe(false);
    expect(result.url).toContain("/auth/callback?token=");
    expect(result.url).toContain("&returnTo=%2Fdossiers%2Fabc");
  });

  it("case 29: unsafe returnTo is silently dropped", async () => {
    const { sendMagicLink } = await import("@/lib/auth/magic-link");
    const result = await sendMagicLink("user@example.com", "//evil.com");
    expect(result.emailSent).toBe(false);
    expect(result.url).toContain("/auth/callback?token=");
    expect(result.url).not.toContain("returnTo");
    expect(result.url).not.toContain("evil.com");
  });

  it("case 30: no returnTo arg preserves backward-compatible URL shape", async () => {
    const { sendMagicLink } = await import("@/lib/auth/magic-link");
    const result = await sendMagicLink("user@example.com");
    expect(result.emailSent).toBe(false);
    expect(result.url).toMatch(/\/auth\/callback\?token=[A-Za-z0-9_-]+$/);
    expect(result.url).not.toContain("returnTo");
  });

  it("case 29b: absolute-URL returnTo is dropped", async () => {
    const { sendMagicLink } = await import("@/lib/auth/magic-link");
    const result = await sendMagicLink("user@example.com", "https://evil.com/steal");
    expect(result.url).not.toContain("returnTo");
    expect(result.url).not.toContain("evil.com");
  });
});

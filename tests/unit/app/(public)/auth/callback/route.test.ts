...
describe("GET /auth/callback", () => {
  ...
  it("case 31: valid token + safe returnTo redirects there", async () => {
    mockConsumeMagicLink.mockResolvedValueOnce({
      session: { sid: "test-sid", cookie: "sealed-cookie" },
      user: { id: "user-1", email: "test@example.com", role: "admin" },
    });
    const response = await GET(makeRequest("?token=valid-token&returnTo=%2Fdossiers%2Fabc"));
    expect((response as any)._redirectUrl).toContain("/dossiers/abc");
    expect((response as any)._redirectUrl).not.toContain("/dashboard");
    expect((response as any)._cookies.bastion_session).toBeDefined();
  });

  it("case 32: valid token + unsafe returnTo falls back to /dashboard", async () => {
    mockConsumeMagicLink.mockResolvedValueOnce({
      session: { sid: "test-sid", cookie: "sealed-cookie" },
      user: { id: "user-1", email: "test@example.com", role: "admin" },
    });
    const response = await GET(makeRequest("?token=valid-token&returnTo=%2F%2Fevil.com"));
    expect((response as any)._redirectUrl).toContain("/dashboard");
    expect((response as any)._redirectUrl).not.toContain("evil.com");
  });

  it("case 32b: valid token + absolute-URL returnTo falls back to /dashboard", async () => {
    mockConsumeMagicLink.mockResolvedValueOnce({
      session: { sid: "test-sid", cookie: "sealed-cookie" },
      user: { id: "user-1", email: "test@example.com", role: "admin" },
    });
    const response = await GET(
      makeRequest("?token=valid-token&returnTo=https%3A%2F%2Fevil.com"),
    );
    expect((response as any)._redirectUrl).toContain("/dashboard");
    expect((response as any)._redirectUrl).not.toContain("evil.com");
  });

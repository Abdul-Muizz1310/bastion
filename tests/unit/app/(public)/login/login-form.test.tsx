...
describe("LoginForm", () => {
  ...
  it("case 37: renders hidden returnTo input when returnTo prop provided", () => {
    const html = renderToString(
      createElement(LoginForm, { demoMode: false, returnTo: "/dossiers/abc" }),
    );
    expect(html).toContain('name="returnTo"');
    expect(html).toContain('value="/dossiers/abc"');
    expect(html).toContain('type="hidden"');
  });

  it("case 37b: does NOT render hidden returnTo input when prop is absent", () => {
    const html = renderToString(createElement(LoginForm, { demoMode: false }));
